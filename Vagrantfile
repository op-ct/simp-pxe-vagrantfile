...
Vagrant.configure("2") do |config|
  ...
  config.vagrant.plugins = ['vagrant-bolt']
  ...
  config.vm.define 'puppetserver', primary: true do |v|
    ...
    #
    # (PROTIP: A local `shared` folder isn't necessary for this to work.)
    ...
    v.vm.provider :virtualbox do |vb|
      vb.customize [
                    ...
                    '--audio', 'null', # this is to stop VirtualBox from blocking ALSA
                   ]
    ...
    # Upload ISOs into /var/simp/ISOs
    if SIMP_OPTIONS['os_iso_files'] && !SIMP_OPTIONS['os_iso_files'].empty?
      ...
      SIMP_OPTIONS['os_iso_files'].each do |iso_file, _iso_version|
    ...
    # Run Bolt to configure the Puppetserver after a `vagrant up`
    v.vm.provision :bolt do |bolt|
      bolt.command = :plan
      bolt.name = 'simp_pxe_test::setup_puppetserver'
      bolt.verbose = true
      bolt.debug = true
    end
  end
